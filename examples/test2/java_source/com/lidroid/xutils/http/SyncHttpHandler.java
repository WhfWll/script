...
public class SyncHttpHandler
{
    ...
    throws HttpException, IOException
  {
    ...
    if (i < 300)
    {
      paramHttpResponse = new ResponseStream(paramHttpResponse, charset, requestUrl, expiry);
      paramHttpResponse.setRequestMethod(requestMethod);
    ...
    if ((i == 301) || (i == 302))
    {
      ...
      paramHttpResponse = httpRedirectHandler.getDirectRequest(paramHttpResponse);
      if (paramHttpResponse != null) {
  ...
  {
    HttpRequestRetryHandler localHttpRequestRetryHandler = client.getHttpRequestRetryHandler();
    try
    {
      requestUrl = paramHttpRequestBase.getURI().toString();
      requestMethod = paramHttpRequestBase.getMethod();
      if (HttpUtils.sHttpCache.isEnabled(requestMethod))
      {
        localObject = HttpUtils.sHttpCache.get(requestUrl);
        if (localObject != null) {
          return new ResponseStream((String)localObject);
        }
      }
      Object localObject = handleResponse(client.execute(paramHttpRequestBase, context));
      return localObject;
    }
    catch (UnknownHostException localUnknownHostException)
    {
      do
      {
        i = retriedTimes + 1;
        retriedTimes = i;
        bool = localHttpRequestRetryHandler.retryRequest(localUnknownHostException, i, context);
      } while (bool);
    ...
    catch (IOException localIOException1)
    {
      for (;;)
      {
        i = retriedTimes + 1;
        retriedTimes = i;
        bool = localHttpRequestRetryHandler.retryRequest(localIOException1, i, context);
      }
    }
    catch (NullPointerException localNullPointerException)
    {
      for (;;)
      {
        ...
        i = retriedTimes + 1;
        retriedTimes = i;
        bool = localHttpRequestRetryHandler.retryRequest(localIOException2, i, context);
      }
    }
    ...
    catch (Throwable localThrowable)
    {
      for (;;)
      {
        ...
        int i = retriedTimes + 1;
        retriedTimes = i;
        boolean bool = localHttpRequestRetryHandler.retryRequest(localIOException2, i, context);
      }
    }
  }
