  ...
  implements GLSurfaceView.Renderer
{
  ...
  private VideoRendererGui(GLSurfaceView paramGLSurfaceView)
  {
    ...
    yuvImageRenderers = new ArrayList();
  }
  ...
  public static VideoRendererGui.YuvImageRenderer create(int paramInt1, int paramInt2, int paramInt3, int paramInt4, VideoRendererGui.ScalingType arg4, boolean paramBoolean)
  {
    ...
    VideoRendererGui.YuvImageRenderer localYuvImageRenderer = new VideoRendererGui.YuvImageRenderer(instancesurface, instanceyuvImageRenderers.size(), paramInt1, paramInt2, paramInt3, paramInt4, ???, paramBoolean, null);
    synchronized (instanceyuvImageRenderers)
    {
      CountDownLatch localCountDownLatch;
      if (instanceonSurfaceCreatedCalled)
      {
        localCountDownLatch = new CountDownLatch(1);
        instancesurface.queueEvent(new VideoRendererGui.1(localYuvImageRenderer, localCountDownLatch));
      }
      try
      {
        localCountDownLatch.await();
        instanceyuvImageRenderers.add(localYuvImageRenderer);
  ...
  public static void update(VideoRenderer.Callbacks paramCallbacks, int paramInt1, int paramInt2, int paramInt3, int paramInt4, VideoRendererGui.ScalingType paramScalingType)
  {
    ...
    synchronized (instanceyuvImageRenderers)
    {
      Iterator localIterator = instanceyuvImageRenderers.iterator();
  ...
  public void onDrawFrame(GL10 arg1)
  {
    ...
    {
      Iterator localIterator = yuvImageRenderers.iterator();
  ...
  public void onSurfaceChanged(GL10 arg1, int paramInt1, int paramInt2)
  {
    ...
    synchronized (yuvImageRenderers)
    {
      Iterator localIterator = yuvImageRenderers.iterator();
      if (localIterator.hasNext()) {
        ((VideoRendererGui.YuvImageRenderer)localIterator.next()).setScreenSize(screenWidth, screenHeight);
      }
    }
  }
  ...
  public void onSurfaceCreated(GL10 arg1, EGLConfig paramEGLConfig)
  {
    ...
    yuvProgram = createProgram("varying vec2 interp_tc;\nattribute vec4 in_pos;\nattribute vec2 in_tc;\n\nvoid main() {\n  gl_Position = in_pos;\n  interp_tc = in_tc;\n}\n", "precision mediump float;\nvarying vec2 interp_tc;\n\nuniform sampler2D y_tex;\nuniform sampler2D u_tex;\nuniform sampler2D v_tex;\n\nvoid main() {\n  float y = texture2D(y_tex, interp_tc).r;\n  float u = texture2D(u_tex, interp_tc).r - 0.5;\n  float v = texture2D(v_tex, interp_tc).r - 0.5;\n  gl_FragColor = vec4(y + 1.403 * v,                       y - 0.344 * u - 0.714 * v,                       y + 1.77 * u, 1);\n}\n");
    oesProgram = createProgram("varying vec2 interp_tc;\nattribute vec4 in_pos;\nattribute vec2 in_tc;\n\nvoid main() {\n  gl_Position = in_pos;\n  interp_tc = in_tc;\n}\n", "#extension GL_OES_EGL_image_external : require\nprecision mediump float;\nvarying vec2 interp_tc;\n\nuniform samplerExternalOES oes_tex;\n\nvoid main() {\n  gl_FragColor = texture2D(oes_tex, interp_tc);\n}\n");
    synchronized (yuvImageRenderers)
    {
      paramEGLConfig = yuvImageRenderers.iterator();
      if (paramEGLConfig.hasNext()) {
        VideoRendererGui.YuvImageRenderer.access$800((VideoRendererGui.YuvImageRenderer)paramEGLConfig.next(), yuvProgram, oesProgram);
      }
    }
