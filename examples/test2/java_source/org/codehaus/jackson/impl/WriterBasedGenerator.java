  ...
  extends JsonGeneratorBase
{
  ...
  {
    int[] arrayOfInt = _outputEscapes;
    int i2 = Math.min(arrayOfInt.length, _maximumNonEscapedChar + 1);
    int k = 0;
    int j = 0;
    int i;
    ...
    throws IOException, JsonGenerationException
  {
    ...
    {
      k = 65535;
      i1 = Math.min(arrayOfInt.length, _maximumNonEscapedChar + 1);
      localCharacterEscapes = _characterEscapes;
      j = 0;
    }
    label48:
    int i;
    for (int m = 0;; m = _prependOrWriteCharacterEscape(_outputBuffer, j, paramInt, i, arrayOfInt[i]))
    {
      if (j < paramInt)
      {
        i = _outputBuffer[j];
    ...
    throws IOException, JsonGenerationException
  {
    ...
    int[] arrayOfInt = _outputEscapes;
    int k = Math.min(arrayOfInt.length, _maximumNonEscapedChar + 1);
    if (_outputTail < j) {
      label124:
      do
      {
